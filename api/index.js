...
// kleine Startseite (zum Testen im Browser)
app.get("/", (_req, res) => {
  res.type("text/plain").send("PDF-API läuft ✔");
});

// HTML aus GPT bauen – mit Poppins
...
async function buildContentPdf(html, outPath) {
  ...
  await page.waitForTimeout(300); // Fonts sicher laden
  await page.pdf({
...
// PDFs anhängen (Links bleiben, weil wir Seiten kopieren)
...
app.post("/create-pdf", async (req, res) => {
  try {
    ...
    const downloadUrl = `${proto}://${host}/api/dl/${id}.pdf`;
